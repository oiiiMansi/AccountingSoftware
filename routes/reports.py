...
@reports.route("/api/report/summary", methods=["GET"])
...
@reports.route("/api/report/income-statement", methods=["GET"])
def get_income_statement():
    ...
    cursor.execute("""
        ...
        WHERE date BETWEEN %s AND %s
        UNION ALL
        SELECT 'Non-Billed Purchases' as category, SUM(amount) as amount
        FROM purchases
        WHERE date BETWEEN %s AND %s
    """, (start_date_str, end_date_str, start_date_str, end_date_str, 
          start_date_str, end_date_str, start_date_str, end_date_str))
...
def generate_report_dataframe(report_type, start_date, end_date):
    ...
    if report_type == 'income_statement':
        # Get revenue data
        cursor.execute("""
            SELECT 'Revenue' as category, 'Billed Sales' as subcategory, SUM(total_amount) as amount
            FROM bills
            WHERE date BETWEEN %s AND %s
            UNION ALL
            SELECT 'Revenue' as category, 'Non-Billed Sales' as subcategory, SUM(amount) as amount
            FROM non_billed_sales
            WHERE date BETWEEN %s AND %s
            UNION ALL
            SELECT 'Expenses' as category, category as subcategory, SUM(amount) as amount
            FROM expenses
            WHERE date BETWEEN %s AND %s
            GROUP BY category
            UNION ALL
            SELECT 'Expenses' as category, 'Salary' as subcategory, SUM(amount) as amount
            FROM salary
            WHERE date BETWEEN %s AND %s
            UNION ALL
            SELECT 'Expenses' as category, 'Purchases' as subcategory, SUM(amount * (1 + gst_percentage/100)) as amount
            FROM billed_purchases
            WHERE date BETWEEN %s AND %s
            UNION ALL
            SELECT 'Expenses' as category, 'Non-Billed Purchases' as subcategory, SUM(amount) as amount
            FROM purchases
            WHERE date BETWEEN %s AND %s
        """, (start_date_str, end_date_str, start_date_str, end_date_str, 
              start_date_str, end_date_str, start_date_str, end_date_str, 
